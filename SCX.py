...
def SCX(problem: TravelingSalesPersonProblem, individual1:TravelingSalesPersonIndividual, individual2: TravelingSalesPersonIndividual):
    ...
    while len(offspring) < dimension:
        ...
        if cand1 == cand2:
            ...
            cost_offspring += cost2
            offspring.append(cand2)

    cost_offspring += problem.get_weight(offspring[-1], offspring[0])
    new_individual = TravelingSalesPersonIndividual()
    new_individual.set_order(offspring)
    new_individual.set_cost(cost_offspring)
